...
def generate_training_dataset(input_filepath: str, output_filepath: str):
    with open(input_filepath, 'r', encoding='utf-8') as f:
        dataset = json.load(f)
    training_data = []
    for dialogue in tqdm(dataset, desc="Processing AMI Dialogues"):
        processor = RealTimeDialogueProcessor()
        for index, turn in enumerate(dialogue.get("conversation", [])):
            en_speaker = turn.get("en_speaker", "Unknown")
            ja_speaker = turn.get("ja_speaker", en_speaker)
    ...
    print(f"Total: {len(dataset)} dialogues and {len(training_data)} sentence pairs.")
    with open(output_filepath, 'w', encoding='utf-8') as f:
        json.dump(training_data, f, ensure_ascii=False, indent=2)

if __name__ == "__main__":
    INPUT_FILE  = "ami_train.json"
    OUTPUT_FILE = "mem_ami_train.json"
